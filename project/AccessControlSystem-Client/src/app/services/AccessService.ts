...
export class AccessService {
  ...
  getAllAccesses() {
   var httpOptions = {
    ...
    let actions: Access[] = [];
    return this.http.get<Object[]>('http://35.222.23.193:8083/administrator/api/v1/employee/list/access', httpOptions).pipe(
      map(response => {
  ...
  registerNewAccess(employeeId: number, doorLockId: number) {
    ...
    var path = `http://35.222.23.193:8083/administrator/api/v1/employee/give_access/` + employeeId + '/' + doorLockId + '/';
    console.log(path);
  ...
  removeAnAccess(employeeId: number, doorLockId: number) {
    ...
    var path = `http://35.222.23.193:8083/administrator/api/v1/employee/remove_access/` + employeeId + '/' + doorLockId + '/';
    console.log(path);
    return this.http.post(path, {}, httpOptions)
      .pipe(map(response => {
        return 'Ok';
